    ...
    <view class="scroll-wrap" :style="{zIndex: zIndex}">
        ...
        <view v-if="isUseOpenList" v-show="!isShowList" class="open-list" @click="handleShowList">
            <slot name="icon-unfold">
				<uni-icons class="icon-unfold" type="arrowdown"></uni-icons>
        ...
        <view v-show="isShowList" class="list-wrap bg-fff">
            <slot name="open-list-tit">
                <view class="flex justify-between border-bottom-solid-ccc">
                    ...
                    >
                        <slot name="icon-fold">
							<uni-icons class="icon-fold" type="arrowup"></uni-icons>
            ...
            <view class="flex justify-between flex-wrap padding-lr padding-bottom-sm">
                <view
                    v-for="(item, index) in list"
                    :key="index"
                    :style="choseInd === index ? getActiveStyle : getDefaultStyle"
...
export default {
	...
	created() {
		this.currentIndex = this.currentType
	},
    computed: {
        getDefaultStyle() {
    ...
    methods: {
        ...
        onClickListItem(index, event) {
            ...
            this.choseInd = index
            this.$emit('onValueChange', {value:item,currentIndex:index})
...
.scroll-view-item_wrap:nth-child(4n-3) {
	margin-left: 0;
}
