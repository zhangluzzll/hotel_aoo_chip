						  ...
						  <uni-list-item   v-for="(item, index) in filterList" :key="item.id" class="list-item" >
								<template slot="body">
									<uni-card :title="'订单编号:'+item.id" note="true" extra="" isShadow="true">
										<uni-tag  :text="item.status" mark="true" class="tag" size="small" :type="commonFn.statusShareType(item.status)"></uni-tag>
										<view v-show="item.floor!==''">
											楼层号：<text class="margin-left font-bolder " >{{item.floor}}F</text>														
											   ...
											   <view v-show="item.status!=='待送货'&& item.status!=='已取消'">																				<text >送货司机：{{item.driverName}}</text>
											   </view>
												<view>
												...
												<template v-slot:footer>
													<view class="footer-box">
														<!-- <view v-show="item.status=='待送货'" class="color-yellow" @click="cancelOrder(item)">取消订单</view> -->
														<view v-show="cancel(item)" class="color-yellow" @click="cancelOrder(item)">取消订单</view>
													   <navigator :url="'/pages/order/detail/detail?type=public&&id='+item.id"><view class="color-blue">订单详情</view></navigator>
				...
				<h3>全部筛选条件：</h3>
				 <uni-forms >
						...
						<view class="example-body">
						 ...
						 <!-- <view class="label-title">净布草订单状态</view>
						 <view class="example-body">
	...
	export default {
		...
		data() {
			return {
				...
				activeColor: '#007aff',
				items: ['全部','待送货','送货中','已取消','已完成'],//tab选项 
				current: 0,
				orderList:[[],[],[],[],[]],
				webUrl:"",
		...
		methods: {
			...
			getHotelList(obj){
				...
				api.publicOrderList(this.search).then((res)=>{
			...
			cancel(item){
				// console.log(item)
				if(item.status == '待送货'){
					console.log(item.insertTime)
					let  preDate = new Date(item.insertTime).getTime();
					let timestamp = new Date().getTime(); // 当前时间的时间戳
					let dateDifference =Math.floor((timestamp - preDate)/(3600*1000)) 
			...
			onClickItem(e){
				...
				if (this.current !== e.currentIndex) {
					...
					switch(this.current) {
					 case 1:
					    this.search.status = "TO_SEND"
					    break;
					 case 2:
					    this.search.status = "SENDING"
					    break;
					 case 3:
						this.search.status = "CANCEL"
						break;
					 case 4:
